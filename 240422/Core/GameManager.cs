...
public class GameManager : NetSingleton<GameManager>
{
    ...
    protected override void OnInitialize()
    {
        ...
        // 어떤 클라이언트가 접속/접속해제 했을 때마다 실행 (서버에는 항상 실행, 클라이언트는 자기 것만 실행)
        NetworkManager.OnClientConnectedCallback += OnClientConnect;        // 어떤 클라이언트가 접속할 때마다 실행
        NetworkManager.OnClientDisconnectCallback += OnClientDisconnect;    // 어떤 클라이언트가 접속 해제할 때마다 실행
