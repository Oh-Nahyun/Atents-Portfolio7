...
public class Logger : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // onEndEdit;   // 입력이 끝났을 때 실행 (엔터를 치거나 포커스를 잃었을 때 실행)
        ...
        inputField.onSubmit.AddListener((text) =>
        {
            ...
            // inputField.Select(); // 활성화 되어있을 때는 비활성화, 비활성화 되어있을 때는 활성화
        });
    ...
    string HighlightSubString(string source, char open, char close, Color color)
    {
        ...
        if (IsPair(source, open, close)) // source 문자열 안에 있는 괄호가 쌍으로 잘 맞는지 확인 (완전히 맞을 때만 강조 처리)
        {
            ...
            result = string.Empty;                                  // IsPair가 실해일 때는 result에 source가 있는게 맞기 때문에 여기서 초기화
            for (int i = 0; i < split.Length; i++) // 나누어진 것들을 하나씩 처리
            {
                result += split[i];         // 나누어진 문자열을 result에 추가
                if (i != split.Length - 1)  // 마지막 문자열은 제외하고
                {
                    if (i % 2 == 0)         // i가 짝수이다. == 이 이후에는 괄호가 열렸을 것이다.
                    {
                        result += $"<#{colorText}>";    // 괄호가 열렸을 때부터 색상 변경
                    }
                    else                    // i가 홀수이다. == 이 이후에는 괄호가 닫혔을 것이다.
                    {
                        result += "</color>";           // 색상 변경 정지
                    }
                }
            }
    ...
    /// <summary>
    /// source에 지정된 괄호가 정확한 조건으로 들어있는지 체크하는 함수
    /// </summary>
    /// <param name="source">원문</param>
    /// <param name="open">여는 괄호</param>
    /// <param name="close">닫는 괄호</param>
    ...
    bool IsPair(string source, char open, char close)
    {
        ...
        // count가 0인 경우는 HighlightSubString()에서 변경할 필요가 없으니 false
    ...
    void ConsoleCommand(string command)
    {
        ...
        string commandToken = command.Substring(0, space);  // 첫번째 ~ space 위치 앞까지
        commandToken = commandToken.ToLower();              // 전부 소문자로 바꾸기
        string dataToken = command.Substring(space + 1);    // space 위치 다음 칸 ~ 끝까지
        ...
        switch (commandToken)
        {
            case "/setname":
                ...
                break;
            case "/setcolor":
                string[] colorStrings = dataToken.Split(',', ' '); ///// 구분 문자 설정
